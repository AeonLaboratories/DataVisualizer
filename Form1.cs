...
namespace Data_Visualizer
{
    public partial class Form1 : Form
    {
        ...
        //double xMin0, xMax0, yMin0, yMax0;  // dataset bounds calculated from visible series
                    ...
                    if (token.Length > 0)
                    {
                        ...
                        foreach (string t in token.Skip(xIsOmitted ? 0 : 1))
                        {
                            try
                            {
                                ...
                                if (this.series.Count < series + 1)
                                    this.series.Add(new Series());
                                this.series[series].Add(new WorldPoint(nRecords, x, y));
                                recordAdded = true;
                            }
        ...
        void addPen(Series s)
        {
            int i = pathPen.Count;
            Color c = someColor[i % someColor.Count];
            s.Pen = new Pen(c, 1.0f);
            ...
            if (series.Count > i)
                toolTip.SetToolTip(patch, series[i].Name ?? $"{i}");
        ...
        string[] tokenize(string line)
        {
            ...
            return decommented.Split(delimiters, StringSplitOptions.RemoveEmptyEntries);
        }
        ...
        void plotGraph()
        {
            ...
            if (this.series != null)
            {
                ...
                if (penColorPanel.Controls.Count == 0)
                {
                    ...
                    foreach (Series s in this.series) addPen(s);
                ...
                foreach (Series s in this.series)
                {
                    if (!s.IsVisible)
                        continue;
                    pathPen.Add(s.Pen);
                    canvasData.Add(new Point[s.Data.Count]);
                    int i = 0;
                    foreach (WorldPoint wp in s.Data)
                    {
        ...
        void scaleEverything()
        {
            ...
            yDivisions = yScalePictureBox.Height / (DeviceDpi / 2);
            ...
            xDivisions = (canvasPictureBox.Width + 2 * margin) / DeviceDpi;
        ...
        int nearestRecno(double x, double y)
        {
            ...
            foreach (WorldPoint wp in series[0].Data)
            {
                dx = Math.Abs(x - wp.X);
                if (dx < bestDx)
                {
        ...
        private void canvasPictureBox_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    if (record == null || rec < 0 || record.Count == 0)
                        statusLabel1.Text = "";
                    else
            ...
            if (mouseDown)
            {
                ...
                try
                {
                    if (e.Y >= yScaleMax)
                    {
                        ...
                        descendY = !descendY;
                        descendYMenuItem.Checked = descendY;
                        double yMin0 = series[0].yMin;
                        double yMax0 = series[0].yMax;
                        for (int i = 1; i < series.Count; i++)
                        {
                            if (series[i].yMin < yMin0)
                                yMin0 = series[i].yMin;
                            if (series[i].yMax > yMax0)
                                yMax0 = series[i].yMax;
                        }
            ...
            if (mouseDown)
            {
                ...
                try
                {
                    if (e.X <= xScaleMin)
                    {
                        ...
                        descendX = !descendX;
                        descendXMenuItem.Checked = descendX;
                        xMin = series.Min(s => s.xMin);
                        xMax = series.Max(s => s.xMax);
                    }
                    else
                    {
        ...
        private void yScalePictureBox_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                yMin = series?.Where(s => s.IsVisible)?.Min(s => s.yMin) ?? 0;
                yMax = series?.Where(s => s.IsVisible)?.Max(s => s.yMax) ?? 100;
                plotGraph();
            }
            catch { }
        }
        ...
        private void xScalePictureBox_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                xMin = series?.Where(s => s.IsVisible)?.Min(s => s.xMin) ?? 0;
                xMax = series?.Where(s => s.IsVisible)?.Min(s => s.xMax) ?? 100;
                plotGraph();
            }
            catch { }
        }
        ...
        private void Color_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                {
                    s.Pen.Color = c.BackColor = c.ForeColor = colorDialog1.Color;
                    plotGraph();
                }
            }
            else
            {
                ...
                c.BackColor = s.IsVisible ? s.Pen.Color : Color.Transparent;
                plotGraph();
            }
        }
